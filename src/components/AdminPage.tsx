                    ...
                    <section>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                            ...
                            <div style={{ background: '#eee', padding: '5px', borderRadius: '20px', display: 'flex', gap: '5px' }}>
                                <button onClick={() => setStatsType('count')} style={{ padding: '5px 15px', borderRadius: '15px', border: 'none', cursor: 'pointer', background: statsType === 'count' ? 'white' : 'transparent', boxShadow: statsType === 'count' ? '0 2px 5px rgba(0,0,0,0.1)' : 'none', fontWeight: statsType === 'count' ? 'bold' : 'normal' }}>권수</button>
                                <button onClick={() => setStatsType('length')} style={{ padding: '5px 15px', borderRadius: '15px', border: 'none', cursor: 'pointer', background: statsType === 'length' ? 'white' : 'transparent', boxShadow: statsType === 'length' ? '0 2px 5px rgba(0,0,0,0.1)' : 'none', fontWeight: statsType === 'length' ? 'bold' : 'normal' }}>글자 수</button>
                            </div>
                        </div>

                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '20px' }}>
                            <div className="glass-card" style={{ textAlign: 'center' }}>
                                <div style={{ fontSize: '0.8rem', color: '#666', marginBottom: '10px' }}>{statsType === 'count' ? '총 누적 독서량' : '총 누적 글자 수'}</div>
                                <div style={{ fontSize: '2.5rem', fontWeight: 'bold', color: 'var(--primary)' }}>{statsType === 'count' ? `${totalBooks}권` : `${totalLength.toLocaleString()}자`}</div>
                            </div>
                            <div className="glass-card" style={{ textAlign: 'center' }}>
                                ...
                                <div style={{ fontSize: '1.8rem', fontWeight: 'bold', color: '#e67e22' }}>
                                    ...
                                    <div style={{ fontSize: '0.9rem', color: '#888', marginTop: '5px' }}>{statsType === 'count' ? `${readingKing?.count}권` : `${readingKing?.length.toLocaleString()}자`}</div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    return (
                                        <div key={user.name} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                            ...
                                            <div style={{ flex: 1, background: '#f0f0f0', borderRadius: '10px', height: '16px', overflow: 'hidden' }}>
                                                <div style={{ width: `${barPercent}%`, background: 'var(--primary)', height: '100%', transition: 'width 1s ease-in-out', minWidth: val > 0 ? '5px' : '0' }}></div>
                                            </div>
                                            <div style={{ width: '70px', textAlign: 'right', fontSize: '0.8rem' }}>{statsType === 'count' ? `${val}권` : `${val.toLocaleString()}자`}</div>
                                        </div>
                                    );
                                })}
                            </div>
                                ...
                                {users.map(u => (
                                    <div key={u.id} style={{
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                                            ...
                                            <input type="number" defaultValue={u.age || ''} onBlur={(e) => updateAge(u.id, e.target.value)} placeholder="입력" style={{ width: '50px', padding: '2px 5px', border: '1px solid #eee', borderRadius: '5px', fontSize: '0.9rem' }} onClick={(e) => e.stopPropagation()} />
                            ...
                            {analysisResult && (
                                <div style={{ display: 'grid', gap: '20px', animation: 'fadeIn 0.5s' }}>
                                    {analysisResult.error ? (
                                        <div style={{ padding: '20px', background: '#ffebee', color: '#c62828', borderRadius: '10px' }}>{analysisResult.error}</div>
                                    ) : (
                                        <>
                                            <div style={{ background: '#e3f2fd', padding: '15px', borderRadius: '15px', borderLeft: '5px solid #2196f3' }}>
                                                ...
                                                <p style={{ fontSize: '1rem', margin: 0, lineHeight: '1.6' }}>{analysisResult.level}</p>
                                            </div>
                                            ...
                                            <div style={{ background: '#f3e5f5', padding: '15px', borderRadius: '15px', borderLeft: '5px solid #9c27b0' }}>
                                                ...
                                                <p style={{ fontSize: '1rem', margin: 0, lineHeight: '1.6' }}>{analysisResult.interest}</p>
                                            </div>
                                                    ...
                                                    {Array.isArray(analysisResult.recommendations) ? analysisResult.recommendations.map((book: any, idx: number) => (
                                                        <div key={idx} style={{ background: 'white', padding: '15px', borderRadius: '15px', border: '1px solid #e8ede8', boxShadow: '0 4px 6px rgba(0,0,0,0.02)', display: 'flex', gap: '15px', minWidth: 0, overflow: 'hidden' }}>
                                                            ...
                                                            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '5px' }}>
                                                                <div style={{ fontWeight: 'bold', color: '#2e7d32', fontSize: '1rem', whiteSpace: 'normal', wordBreak: 'break-word', lineHeight: '1.4' }}>{book.title}</div>
                                                                <div style={{ fontSize: '0.85rem', color: '#666' }}>저자: {book.author}</div>
                                                                <p style={{ fontSize: '0.9rem', color: '#444', lineHeight: '1.6', margin: '5px 0 0 0', flex: 1 }}>{book.reason}</p>
                                                            </div>
                                                        </div>
                                                    )) : <p>추천 정보가 없습니다.</p>}
                                                </div>
